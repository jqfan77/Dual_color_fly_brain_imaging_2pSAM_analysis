...
# pca in each block
def pca_block(a,ap,channel_selected,num_tp,num_trial,mask,thresh_pca_exp_var_1,pca_tp_range,pca_data_c0,pca_data_c0_p,block_dim):
    a_c0 = a[:,:,:,:,:,channel_selected]
    a_c0 = a_c0.reshape((-1,num_tp*num_trial),order = 'F')
    a_c0 = np.transpose(a_c0,[1,0])
    a_c0 = a_c0[:,mask]
    
    ap_c0 = ap[:,:,:,:,:,channel_selected]
    ap_c0 = ap_c0.reshape((-1,num_tp*num_trial),order = 'F')
    ap_c0 = np.transpose(ap_c0,[1,0])
    ap_c0 = ap_c0[:,mask]
    ...
    if np.size(a_c0,1)>0:
        ...
        ap_c0=scale(X=ap_c0,with_mean=True,with_std=True,copy=True)
        b_c0=scale(X=b_c0,with_mean=True,with_std=True,copy=True)
        pca = PCA()
        pca.fit(b_c0)
        y = pca.transform(a_c0)
        yp = pca.transform(ap_c0)
        ...
        if not len(pca_data_c0):
            ...
            pca_data_c0_p = yp[:,range(num_dim)]
        else:
            pca_data_c0  = np.concatenate((pca_data_c0,y[:,range(num_dim)]),axis = 1)  
            pca_data_c0_p  = np.concatenate((pca_data_c0_p,yp[:,range(num_dim)]),axis = 1)  
        block_dim.append(num_dim)
    return pca_data_c0,pca_data_c0_p,block_dim
# block-wise pca for voxel-level multiple-brain-region classification; and pca using the consolidated PCs of each block
def fly_pca_block_check_denoise(data,data_nodenoise,atlas,atlas_c1,atlas_c2,atlas_plus,stim,channel_selected,odor_choice,
...
# atlas: size_x,size_y,size_z
    ...
    data_nodenoise = data_nodenoise[:,:,:,:,stim!=odor_choice,:]
    ...
    if channel_selected != 0:
        ...
        data_nodenoise = data_nodenoise[:,:,:,:,:,channel_selected-1]
    # imresize-downsample
    if data.ndim == 5:
        data = data[:,:,:,:,:,np.newaxis]
        data_nodenoise = data_nodenoise[:,:,:,:,:,np.newaxis]
    ...
    data_nodenoise[where_are_nan] = 0
    del where_are_nan
    where_are_inf = np.isinf(data)
    data_nodenoise[where_are_inf] = 0
    ...
    for i in range(num_channel): 
        ...
        data_nodenoise[m>dff0_thresh,:,:,i] = 0
    ...
    if channel_selected != 0:
        if channel_selected==1 and os.path.exists(savepath_common + '/' + 'pca_data_c0.npy'):
            ...
            pca_data_c0_p = np.load(savepath_common + '/' + 'pca_data_c0_p.npy')
            ...
            for i in range(num_block_xy):
                ...
                for j in range(num_block_xy):
                    ...
                    for k in range(num_block_z):
                        ...
                        a = a[:,range_y,:,:,:,:]
                        a = a[:,:,range_z,:,:,:]
                        ap = data_nodenoise[range_x,:,:,:,:,:]
                        ap = ap[:,range_y,:,:,:,:]
                        ap = ap[:,:,range_z,:,:,:]
                        mask = mask.reshape(-1,order = 'F')
                        [pca_data_c0,pca_data_c0_p,block_dim] = pca_block(a,ap,0,num_tp,num_trial,mask,thresh_pca_exp_var_1,
                                                                          pca_tp_range,pca_data_c0,pca_data_c0_p,block_dim)
                print('Row ' + str(i) + 'Done!')
        print('C'+ str(channel_selected-1) +' Dim:') 
        print(np.size(pca_data_c0,1)) 
        x_origin = pca_data_c0
        x_origin_p = pca_data_c0_p
    else:
        pca_data_c0 = []
        pca_data_c1 = []
        pca_data_c0_p = []
        ...
        for i in range(num_block_xy):
            ...
            for j in range(num_block_xy):
                ...
                for k in range(num_block_z):
                    ...
                    # channel 1
                    mask = mask.reshape(-1,order = 'F')
                    [pca_data_c0,pca_data_c0_p,block_dim_c0] = pca_block(a,ap,0,num_tp,num_trial,mask,thresh_pca_exp_var_1,pca_tp_range,pca_data_c0,pca_data_c0_p,block_dim_c0)
                    # channel 2
                    mask = atlas_mask_plus[range_x,:,:]
                    mask = mask[:,range_y,:]
                    mask = mask[:,:,range_z]
                    mask = mask.reshape(-1,order = 'F')
                    [pca_data_c1,pca_data_c1_p,block_dim_c1] = pca_block(a,ap,1,num_tp,num_trial,mask,thresh_pca_exp_var_1,pca_tp_range,pca_data_c1,pca_data_c1_p,block_dim_c1)
            print('Row ' + str(i) + 'Done!')
        print('C0 Dim:') 
        print(np.size(pca_data_c0,1)) 
        print('C1 Dim:')
        ...
        x_origin_p=np.concatenate((pca_data_c0_p,pca_data_c1_p),axis = 1)
    ...
    x_pca_nodenoise = pca.transform(x_origin_p)
    # # plot pca projection
    ...
    return x_origin,x_origin_p,block_dim,stim_origin,stim_shuffled,num_tp,num_trial,x_pca,x_pca_nodenoise,explained_variance_ratio
# lda and svm of the trials
def lda_trial(xp,x_lda,train,test,stim,num_tp,num_trial,expr_thresh,kk,odor_choice,c,
    ...
    x_train = clf.transform(xp[train,:])
...
def odor_classification(x_origin,x_origin_nodenoise,explained_variance_ratio,stim,
    ...
    xp = x_origin_nodenoise[:,range(num_dim)]
    print(np.shape(x))
    ...
    for train, test in kf.split(np.linspace(0,num_trial-1,num_trial)):
        [acc, f1_weighted, area] = lda_trial(xp,x_lda,train,test,stim,num_tp,num_trial,expr_thresh,kk,odor_choice,c,
                                             lda_tp_selected,svm_tp_range,if_plot_figure,if_save_figure,savepath)
        # update list
    ...
    return num_dim,list_accuracy,list_f1_weighted,list_AUC_weighted
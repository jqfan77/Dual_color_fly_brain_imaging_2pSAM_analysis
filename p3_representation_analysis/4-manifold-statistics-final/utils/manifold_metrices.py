...
def plot_bar(data,color_list,if_save,save_path,x_label,y_label,y_lim,if_paired_test):
    ...
    for i in range(np.size(data,0)-1):
        for j in range(i+1,np.size(data,0)):
            a = data[i]
            b = data[j]
            if if_paired_test:
                res = wilcoxon(a,b)
            ...
            if p<0.05 and p>=0.01:
                plt.text(0.5*(i+j),np.mean(data[0]),'*',verticalalignment = 'center', horizontalalignment = 'center')

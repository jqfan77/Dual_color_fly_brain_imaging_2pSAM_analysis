...
def pca_block(a,channel_selected,num_tp,num_trial,mask,thresh_pca_exp_var_1,pca_tp_range,pca_data_c0,block_dim):
    ...
    if np.size(a_c0,1)>0:
        ...
        m = pca.components_
        m = m[range(num_dim),:]
    return pca_data_c0,block_dim,m
...
# atlas: size_x,size_y,size_z
            ...
            for i in range(num_block_xy):
                ...
                for j in range(num_block_xy):
                    ...
                    for k in range(num_block_z):
                        ...
                        [pca_data_c0,block_dim,m_pca] = pca_block(a,0,num_tp,num_trial,mask,thresh_pca_exp_var_1,pca_tp_range,pca_data_c0,block_dim)
                        np.save(savepath + '/' +'block_m_pca_channel_'+str(channel_selected)+'_'+str(i)+'_'+str(j)+'_'+str(k)+'.npy',m_pca)
                print('Row ' + str(i) + 'Done!')
        ...
        for i in range(num_block_xy):
            ...
            for j in range(num_block_xy):
                ...
                for k in range(num_block_z):
                    ...
                    # channel 1
                    mask = mask.reshape(-1,order = 'F')
                    [pca_data_c0,block_dim_c0,m_pca] = pca_block(a,0,num_tp,num_trial,mask,thresh_pca_exp_var_1,pca_tp_range,pca_data_c0,block_dim_c0)
                    np.save(savepath + '/' +'block_m_pca_channel_0_'+str(i)+'_'+str(j)+'_'+str(k)+'.npy',m_pca)
                    # channel 2
                    mask = atlas_mask_plus[range_x,:,:]
                    mask = mask[:,range_y,:]
                    mask = mask[:,:,range_z]
                    mask = mask.reshape(-1,order = 'F')
                    [pca_data_c1,block_dim_c1,m_pca] = pca_block(a,1,num_tp,num_trial,mask,thresh_pca_exp_var_1,pca_tp_range,pca_data_c1,block_dim_c1)
                    if len(m_pca)>0:
                        np.save(savepath + '/' +'block_m_pca_channel_1_'+str(i)+'_'+str(j)+'_'+str(k)+'.npy',m_pca)
            print('Row ' + str(i) + 'Done!')
        print('C0 Dim:') 
        print(np.size(pca_data_c0,1)) 
        print('C1 Dim:')
    ...
    explained_variance_ratio = pca.explained_variance_ratio_
    m_pca_all = pca.components_
    np.save(savepath + '/' +'all_m_pca.npy',m_pca_all)
...
def lda_trial(x,x_lda,train,test,stim,num_tp,num_trial,expr_thresh,kk,odor_choice,c,
    ...
    coef = clf.coef_
    np.save(savepath + '/' + 'lda_coef' + '_expr_thresh_' + str(expr_thresh)+'_fold_'+ str(kk) +'.npy',coef)

...
def compare_data_channel(result_path,file_folder_name,file_name,fly_selected,flag,channel,the_label,ymin,ymax,dim_thresh,
    ...
    for j in range(num_channel_choice-1):
        if test_way == 'wilcoxon':
            res = wilcoxon(list_acc[:,num_channel_choice-1],list_acc[:,j])
        else:
    ...
    if if_save:
        ...
        plt.savefig(figure_save_path + '/' + '5E_'+ figure_save_name +'.pdf',dpi = 300,bbox_inches = 'tight')
        plt.savefig(figure_save_path + '/' + '5E_'+ figure_save_name +'.png',dpi = 300,bbox_inches = 'tight')
    plt.show()

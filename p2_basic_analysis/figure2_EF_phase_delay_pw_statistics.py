## 
## This code is used for calculate the statistics of phase and pw for each indicator
##
...
data_g7f = np.nanmean(data_g7f,1)
data_ach = np.nanmean(data_ach,1)
data_5ht = np.nanmean(data_5ht,1)
...
for i in range(3):
    ...
    num_fly = np.size(a,0)
    the_mean = np.mean(a)
    the_std = np.std(a,ddof = 1)/math.sqrt(num_fly)
...
if if_save:
    ...
    plt.savefig(figure_save_path + '/' + filename+'.pdf',bbox_inches = 'tight')
    plt.savefig(figure_save_path + '/' + filename+'.png',bbox_inches = 'tight')
plt.show()

...
# logger
log_file_name = 'brain-network-ach-for-ach-flies-RS'
...
for data_index in data_index_list: 
    ...
    path_output = '/sam2p/0-LLB/olfactory_representation/Ach-' + data_index + '-trans-pre_stimu/'
    ...
    for region_idx in focus_regions:
        ...
        length = int(len(records[0]) / 2) + 1
        records_down = []
        for neu_records in records:
            neu_records_down = [neu_records[2 * i] for i in range(length)]
            records_down.append(neu_records_down)
        logger.info("G7f data down: {} {}".format(len(records_down), len(records_down[0])))
        np.save(path_output +'neuron_concat_records/region_' + str(region_idx) + '_neuron_records_down.npy', records_down)
        
        ## select records for the given time period
        select_data = np.array(records_down)[:, 4 : length - 4]

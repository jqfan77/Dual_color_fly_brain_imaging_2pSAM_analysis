...
# logger
log_file_name = 'temporal-network-5ht'
...
for period_i in range(period_cnt):
    ...
    for fly_idx in data_index_list:
        inputpath = path + fly_idx + '-trans-stimu/'
...
for period_i in range(4):
    ...
    file_name = calc_path +'whole_brain_network_edges' + str(round(1 - delete_seg, 1)) + '_stat_5HT_preiod' + str(period_i) + '.xlsx'
